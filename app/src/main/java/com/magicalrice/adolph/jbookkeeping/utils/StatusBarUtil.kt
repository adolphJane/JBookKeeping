...
object StatusBarUtil {
    ...
    private val flymePattern = Pattern.compile("Flyme OS [4|5]", Pattern.CASE_INSENSITIVE)
    private val flymeOSPattern = Pattern.compile("Flyme", Pattern.CASE_INSENSITIVE)

    //判断是否Flyme4以上
    val isFlyme4Later: Boolean
        get() = (Build.FINGERPRINT.contains("Flyme_OS")
                || Build.VERSION.INCREMENTAL.contains("Flyme_OS")
                || flymePattern.matcher(Build.DISPLAY).find()
                || flymeOSPattern.matcher(Build.DISPLAY).find()
